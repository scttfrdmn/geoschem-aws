...
# Data sources for availability zones (exclude LAX zones to avoid network border group issues)
...
# Security Group for EC2 builder instances
resource "aws_security_group" "ec2_builder" {
  name_prefix = "${var.name_prefix}-ec2-builder"
  vpc_id      = aws_vpc.main.id
  description = "Security group for EC2 builder instances"
  ...
  tags = merge(var.tags, {
    Name = "${var.name_prefix}-ec2-builder-sg"
  })
}

# Security Group for Batch compute environments
resource "aws_security_group" "batch_compute" {
  name_prefix = "${var.name_prefix}-batch-compute"
  vpc_id      = aws_vpc.main.id
  ...
  tags = merge(var.tags, {
    Name = "${var.name_prefix}-batch-compute-sg"
  })
}